...
;; "abc def4 sjdkfj-$$$*&@98123." --> ("abc" "def4" "sjdkfj-$$$*&@98123.")
;(define a-token (many1 (none-of " \r\n\t\v\f")))
;(define tokens (*> opt-whitespace (sep-end-by a-token whitespace)))

;; Tokens for s-expression language:
;; - symbols: "abl_kjkcjxc44-fsijofdj"
;; - numbers: "-4237298"
;; - parentheses: "(", ")"
;; - strings: "\"foo\"" (IGNORE)

;;; Generates a tokenizer given a tokenizer spec.
;;; A tokenizer spec is of the form:
;;;
;;;   ((regex proc)*)
;;;
;;; The regexes should be strings, not actual regex objects. They are
;;; interpreted in racket's pregexp syntax.
;;;
;;; The regexes are matched against the input repeatedly until there is no more
;;; input. If at any time either zero or more than one regexes match, an error
;;; occurs (zero -> "invalid"; more than one -> "ambiguous").
;;;
;;; The proc corresponding with a regex is used to handle the case where that
;;; regex matches. The result of the match is fed to proc, which should output a
;;; list of tokens. These tokens are added to the tokenizer output.
(define (tokenizer ts)
  ;; We do this in the stupid way.
  (let* ((re-strs (map cadr ts))
         (res (map pregexp re-strs))
         (re (pregexp ))
        ))
  )


;;; Unfinished special-case tokenizer for s-expressions
(define sexp-token
  (choice
    symbol-token
    number-token
...
(define symbol-token
  (<$> string->symbol
    (str-many1 (choice alpha digit (any-of "_!@#$%^&*")))))

(define number-token
  (<$> (lambda (f x) (f x))
    (option + (*> (any-of "-") (return -)))
    (<$> string->number (str-many1 digit))))
...
(define p-exps
  ...
  ;; unambiguous. (Really I should just tokenize.)
      (sep-end-by p-sexp opt-whitespace)))
